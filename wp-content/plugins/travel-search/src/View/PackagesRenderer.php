...
{
    /**
     * Render results list (used by shortcode & REST).
     */
    public function render(WP_Query $query, string $singular = 'Trip', string $plural = 'Trips'): string
    {
        ob_start();
        ?>
        <div class="package-results-wrapper">
            <?php if ($query->have_posts()) : ?>
                <h2 class="packages-title">
                    <?php
                    $found = (int) $query->found_posts;
                    echo esc_html($found . ' ' . ($found === 1 ? $singular : $plural) . ' found');
                    ?>
                </h2>
                ...
                <div class="packages-grid">
                    <?php while ($query->have_posts()) : $query->the_post();
                        ...
                        $discount = get_field('discount') ?: '15% OFF';
                    ...
                    ?>
                        <article class="travel-package-card">
                                ...
                                <div class="card-overlay">
                                    <div class="card-left">
                                        ...
                                        <div class="card-info">
                                            ...
                                            <?php endif; ?>
                                            <?php if ($price): ?>
                                                <span> | From <?php echo esc_html($price); ?></span>
                                            <?php endif; ?>
                                        </div>
                                    </div>
                                    ...
                                    <a href="<?php echo esc_url(get_permalink()); ?>" class="card-button">
                                        <?php echo esc_html__('View Trip', 'travel-search'); ?>
                                    </a>
                                </div>
                            </div>
                        </article>
            ...
            <?php else : ?>
                <h2 class="packages-title no-results">
                    <?php echo esc_html__('No trips found', 'travel-search'); ?>
                </h2>
            <?php endif; ?>
        </div>
        <?php
