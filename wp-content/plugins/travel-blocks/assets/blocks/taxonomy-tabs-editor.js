...
(function($) {
    ...
    /**
     * Rebuild select with ALL available terms (no filtering)
     */
    function rebuildSelect($select, selectedIds) {
        ...
        // Restore value if it exists in the options
        const valueExists = allOptions[selectKey].some(opt => opt.value === currentValue);
    ...
    /**
     * Rebuild select with all available terms
     * Shows all taxonomy terms regardless of checkbox selection
     */
    ...
    acf.addAction('ready', function() {
        // Only run if we're editing a Taxonomy Tabs block
        const isTaxonomyTabsBlock = $('[data-name="tt_tab_overrides"]').length > 0;

        if (!isTaxonomyTabsBlock) {
            console.log('TaxonomyTabs: Not a Taxonomy Tabs block, skipping initialization');
            return;
        }

        console.log('TaxonomyTabs: ACF Ready - Initializing filter');

        // Initial filter with delay to ensure DOM is ready
        setTimeout(filterTermSelect, 500);

        // Check for changes every 1 second (more responsive)
    ...
    acf.addAction('change', function(e) {
        // Only run if we're editing a Taxonomy Tabs block
        if ($('[data-name="tt_tab_overrides"]').length === 0) {
            return;
        }
