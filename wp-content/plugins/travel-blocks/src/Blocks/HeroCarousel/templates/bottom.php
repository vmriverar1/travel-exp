...
/**
 * Template: Bottom
 * Layout: Hero text on top, cards at bottom with hero background
 */

// Variables available from render_block:
// $hero_image, $hero_content (InnerBlocks), $has_hero_text, $cards, $columns_desktop, $hero_height_mobile, $hero_height_tablet, $hero_height_desktop, $show_arrows, $show_dots, $enable_autoplay, $autoplay_delay, $is_carousel
// $button_color_variant, $badge_color_variant, $text_alignment, $button_alignment, $cards_negative_margin_top, $cards_negative_margin_bottom, $cards_height, $cards_width
...
/* Cards Negative Margins */
<?php if ($cards_negative_margin_top > 0): ?>
#<?php echo esc_attr($carousel_id); ?> .hc-cards-wrapper {
    margin-top: -<?php echo esc_attr($cards_negative_margin_top); ?>px !important;
}
<?php endif; ?>
...
#<?php echo esc_attr($carousel_id); ?> .hc-cards-wrapper {
    position: relative;
    transform: translateY(<?php echo esc_attr($cards_negative_margin_bottom); ?>px);
}
<?php endif; ?>
</style>
...
$class_to_add = 'hc-hero-carousel hc-hero-carousel--bottom text-align-' . esc_attr($text_alignment) . ' button-align-' . esc_attr($button_alignment);
...
$wrapper_attrs .= ' data-variation="bottom"'; // Help React identify this as a different component
foreach ($carousel_attrs as $key => $value) {
    $wrapper_attrs .= ' ' . esc_attr($key) . '="' . esc_attr($value) . '"';
}
?>

<div <?php echo $wrapper_attrs; ?> key="hero-carousel-bottom">
    ...
    <!-- Content Container -->
    <div class="hc-content <?php echo $has_hero_text ? 'hc-content--with-text' : 'hc-content--centered'; ?>">

        <!-- Hero Text at Top (InnerBlocks) -->
        <div class="hc-hero-text" style="max-width: <?php echo esc_attr($content_proportion * 16); ?>px;">
            <?php if ($is_preview): ?>
                <InnerBlocks />
            <?php else: ?>
        ...
        <div class="hc-cards-wrapper">
                ...
                <!-- Navigation Arrows (only if carousel) -->
