...
/**
 * Template: Side Right
 * Layout: Cards on right side with half hidden, navigation arrows on left
 */
...
#<?php echo esc_attr($carousel_id); ?> .hc-cards-section {
    margin-right: -<?php echo absint($negative_margin); ?>px !important;
}
<?php endif; ?>
...
$class_to_add = 'hc-hero-carousel hc-hero-carousel--side-right text-align-' . esc_attr($text_alignment) . ' button-align-' . esc_attr($button_alignment);
...
$wrapper_attrs .= ' data-variation="side_right"'; // Help React identify this as a different component
foreach ($carousel_attrs as $key => $value) {
    $wrapper_attrs .= ' ' . esc_attr($key) . '="' . esc_attr($value) . '"';
}
?>

<div <?php echo $wrapper_attrs; ?> key="hero-carousel-side-right">
