...
// Get settings from data - Desktop
$image_position = $data['image_position'] ?? 'left';
$image_width = $data['image_width'] ?? 40;
$image_border_radius = $data['image_border_radius'] ?? 12;
$image_min_width = $data['image_min_width'] ?? 0;
$card_min_height = $data['card_min_height'] ?? 450;

// Get settings from data - Mobile
$mobile_image_width = $data['mobile_image_width'] ?? 40;
$mobile_card_height = $data['mobile_card_height'] ?? 180;
$mobile_image_min_width = $data['mobile_image_min_width'] ?? 0;
...
<section
    ...
    style="--image-width: <?php echo esc_attr($image_width); ?>%; --image-min-width: <?php echo esc_attr($image_min_width); ?>px; --image-border-radius: <?php echo esc_attr($image_border_radius); ?>px; --card-gap: <?php echo esc_attr($card_gap); ?>px; --card-min-height: <?php echo esc_attr($card_min_height); ?>px; --grid-columns: <?php echo esc_attr($grid_columns); ?>; --mobile-image-width: <?php echo esc_attr($mobile_image_width); ?>%; --mobile-card-height: <?php echo esc_attr($mobile_card_height); ?>px; --mobile-image-min-width: <?php echo esc_attr($mobile_image_min_width); ?>px;">
    ...
    <div class="sbs-wrapper">
        ...
        <div class="sbs-container">
            <?php foreach ($cards as $index => $card):
                ...
                // Check both 'excerpt' and 'description' keys (manual cards use 'excerpt', dynamic use both)
                ...
                // Ensure column_span is always a valid integer
                $column_span = isset($card['column_span']) && is_numeric($card['column_span']) ? (int)$card['column_span'] : 1;
            ...
            <article
                ...
                <div class="sbs-card__content">
                    ...
                    <!-- Excerpt (only show if column_span > 1) -->
                    <?php if ($excerpt && $column_span > 1): ?>
                        <p class="sbs-card__excerpt"><?php echo esc_html($excerpt); ?></p>
                    <?php endif; ?>

                    <!-- Divider Line -->
                    <?php if (($title || ($excerpt && $column_span > 1)) && ($is_package ? $duration_price : ($location || $price))): ?>
